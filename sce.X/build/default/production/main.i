...
unsigned char buffer[30*5];
unsigned char * ptr_read_buff = ((void*)0);
unsigned char * ptr_write_buff = ((void*)0);
...
void Pull_ring_buffer(){


}


void Store_EEPROM(unsigned char * ptr_buffer_r){

    for(int j =1; j <= 5 && ptr_EEPROM != 0xF0FF ; j++){
...
void Push_ring_buffer( unsigned char reg[5]){

    if (ptr_write_buff == &(buffer[30*5 -1])){

        Store_EEPROM(ptr_read_buff);

    }

    else if (ptr_write_buff == ((void*)0) && ptr_read_buff == ((void*)0)){
